...
class STDN_NAS_Supernet(nn.Module):
    def __init__(self, lstm_seq_len):
        super(STDN_NAS_Supernet, self).__init__()
...
class STDN_NAS_Network(nn.Module):
    def __init__(self, lstm_seq_len, choice):
        super(STDN_NAS_Network, self).__init__()
        ...
        for level_num in range(self.level):
            ...
            flow_block=nn.ModuleList([])
            vol_block=nn.ModuleList([])
            vol_block.append(CNN_Vol(nbhd_inp,nbhd_oup,self.kernel_list[choice[level_num]], self.stride))
            flow_block.append(CNN_Flow(flow_inp, flow_oup, self.kernel_list[choice[level_num]], self.stride))
            self.choice_block.append(nn.ModuleList([vol_block, flow_block]))

    def __call__(self, nbhd_input, flow_input):
        y=self.forward(nbhd_input, flow_input)
        return y

    def forward(self, nbhd_input, flow_input):
            nbhd_input=[nbhd_input[i].to(self.device) for i in range(self.lstm_seq_len)]
            flow_input=[flow_input[i].to(self.device) for i in range(self.lstm_seq_len)]
        ...
        # level 1 with lstm_seq_len model
            nbhd_convs=[self.choice_block[0][0][0](nbhd_input[i]) for i in range(self.lstm_seq_len)]
            nbhd_convs=[self.relu_layer(nbhd_convs[i]) for i in range(self.lstm_seq_len)]

            expected_shape=(nbhd_convs[0].shape[-2],nbhd_convs[0].shape[-1])

            flow_convs=[self.choice_block[0][1][0](flow_input[i]) for i in range(self.lstm_seq_len)]
            flow_convs=[self.relu_layer(flow_convs[i]) for i in range(self.lstm_seq_len)]
        ...
        # level 2 with lstm_seq_len_model
            nbhd_convs=[self.choice_block[1][0][0](nbhd_convs[i]) for i in range(self.lstm_seq_len)]
            nbhd_convs=[self.relu_layer(nbhd_convs[i]) for i in range(self.lstm_seq_len)]

            expected_shape=(nbhd_convs[0].shape[-2],nbhd_convs[0].shape[-1])

            flow_convs=[self.choice_block[1][1][0](flow_input[i]) for i in range(self.lstm_seq_len)]
            flow_convs=[self.relu_layer(flow_convs[i]) for i in range(self.lstm_seq_len)]
        ...
        # level 3 with lstm_seq_len_model
            nbhd_convs=[self.choice_block[2][0][0](nbhd_convs[i]) for i in range(self.lstm_seq_len)]
            nbhd_convs=[self.relu_layer(nbhd_convs[i]) for i in range(self.lstm_seq_len)]

            expected_shape=(nbhd_convs[0].shape[-2],nbhd_convs[0].shape[-1])

            flow_convs=[self.choice_block[2][1][0](flow_input[i]) for i in range(self.lstm_seq_len)]
            flow_convs=[self.relu_layer(flow_convs[i]) for i in range(self.lstm_seq_len)]
