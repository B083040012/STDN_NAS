...
class ASAGA_Searcher():
    ...
    def search(self):
        ...
        for gen in range(self.generation_num):
            for loop in range(self.population_num/2):
            ...
            if global_best_loss<tmp_best_loss:
                ...
                self.logger.info("%%%%%%%%%%%%%%%%%%%%%%%%")
            if gen%10==0:
                self.logger.info("[Generation %d] tmp:%.5f, tmp_best:%.5f, gloabl_best_loss:%.5f" %(gen, self.curr_tmp, tmp_best_loss, global_best_loss))
            self.curr_tmp=self.curr_tmp*self.annealing_ratio
        self.logger.info("--------------[Generation End]--------------")
        self.logger.info("[Searched Architecture] gloabl_best_loss:%.5f" %(global_best_loss))

        return global_best_architecture
    ...
    def selection(self, parent_subfitness, offspring_subfitness, parent_population, offspring_list, index_list):
        ...
        for i in range(len(parent_subfitness)):
            ...
            if parent_subfitness[i]>offspring_subfitness[i]:
                parent_population[index_list[i]]=offspring_list
        ...
        for index, architecture in enumerate(architecture_list):
            ...
            with torch.no_grad():
                for (nbhd, flow, label) in val_loader:
                    ...
                    total_loss, validlen=eval_all(output, target, criterion, self.config["dataset"]["threshold"])
                    if total_loss==-1:
                        continue
